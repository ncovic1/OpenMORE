...
int main(int argc, char **argv)
{
  ...
  YAML::Node node { YAML::LoadFile(project_path + "/replanners_benchmark/config/scenario1.yaml") };
	std::cout << "Scenarios file path: " << project_path + "/replanners_benchmark/config/scenario1.yaml" << "\n";

  for(const std::string replanner_type : replanner_type_vector)
  {
    size_t num_obs = node["environment"].size();
    ...
    for(int i = n_query_start; i < n_query; i++)
    {
      ...
      {
        start_conf(ii) = node["robot"]["start"][ii].as<float>();
        goal_conf(ii) = node["robot"]["goal"][ii].as<float>();
      }
      ...
      Eigen::Vector3d pos {}, dim {};
      for (size_t j = 0; j < num_obs; j++)
      {
        YAML::Node obstacle = node["environment"][j];
        YAML::Node d = obstacle["box"]["dim"];
        YAML::Node p = obstacle["box"]["pos"];
        
        dim = Eigen::Vector3d(d[0].as<float>(), d[1].as<float>(), d[2].as<float>());
        pos = Eigen::Vector3d(p[0].as<float>(), p[1].as<float>(), p[2].as<float>());
